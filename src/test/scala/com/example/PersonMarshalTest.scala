...
import spray.http._
import MediaTypes._
import com.example.PersonMarshaller.`application/vnd.acme.person`
...
class PersonMarshalTest extends FlatSpec with Matchers {
  "Marshal Person with vnd.acme.person" should "result in an HttpEntity with media type vnd.acme.person" in {
    ...
    import com.example.PersonMarshaller.VndAcmeMarshaller
    ...
    marshal(Person("Bob", "Parr", 32))(marshaller = com.example.PersonMarshaller.VndAcmeMarshaller) == Right(HttpEntity(`application/vnd.acme.person`, "Person: Bob, Parr, 32"))
  }
  ...
  "UnMarshal Person with application/xml" should "result in a Person case class" in {
    import com.example.PersonMarshaller.XmlUnmarshaller
    val xmlString =
      <person>
        <name>Bob</name>
        <first>Parr</first>
  ...
  "UnMarshal Person with text/xml" should "result in a Person case class" in {
    ...
    body.as[Person] match {
      case Right(Person("Bob", "Parr", 32)) =>
      case _ => fail("")
    }
  }

  "Marshal Person with application/json" should "result in a JSON format" in {
    // bringing the custom PersonJsonProtocol and spray-json implicits into scope (of the compiler)
    import PersonMarshaller.PersonJsonProtocol._
    import spray.json._
    val json = Person("Bob", "Parr", 32).toJson
    assert(json.toString === """{"name":"Bob","firstName":"Parr","age":32}""")
  }

  "Unmarshal Person with application/json" should "result in a Person case class" in {
    // bringing the custom PersonJsonProtocol and spray-json implicits into scope (of the compiler)
    import PersonMarshaller.PersonJsonProtocol._
    import spray.json._
    """{"name":"Bob","firstName":"Parr","age":32}""".parseJson.convertTo[Person] match {
      case Person("Bob", "Parr", 32) =>
      case _ => fail
    }
  }
}
